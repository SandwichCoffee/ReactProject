...
export default function Home() {
  ...
  const products = productData?.list
    ? [...productData.list].sort((a, b) => b.prodId - a.prodId).slice(0, 4)
    : [];
                  ...
                  >
                    <div className="flex flex-col items-center justify-center w-12 h-12 bg-white border border-slate-200 rounded-lg shadow-sm shrink-0">
                      <span className="text-xs text-slate-500 font-medium">
                        {recruit.startDate?.substring(5, 7)}월
                      </span>
                      <span className="text-sm font-bold text-slate-800">
                        {recruit.startDate?.substring(8, 10)}
                      </span>
                    </div>
              ...
              >
                <div className="aspect-square bg-slate-100 relative overflow-hidden">
                  ...
                  <div className="absolute left-2 top-2">
                    <Badge
                      variant="secondary"
                      className="bg-white/90 backdrop-blur text-slate-800 shadow-sm hover:bg-white"
                    >
