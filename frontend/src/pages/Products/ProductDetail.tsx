...
import { deleteProduct, webhookSimulation } from "@/api/productApi";
...
export default function ProductDetail() {
  ...
  const handleDelete = async () => {
    ...
    deleteMutation.mutate(product.prodId);
  };
                ...
                <Dialog>
                  <DialogTrigger asChild>
                    <Button variant="destructive" className="gap-2" disabled={deleteMutation.isPending}>
                      <Trash2 size={16} /> {deleteMutation.isPending ? "삭제 중..." : "삭제"}
                    </Button>
                  </DialogTrigger>
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        정말 이 상품을 삭제하시겠습니까? 삭제 후에는 복구할 수
                        없습니다.
                      </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
        ...
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-lg text-indigo-900">
              외부 결제 연동 시뮬레이터 (포트폴리오 테스트용)
            </CardTitle>
          </div>
          <CardDescription className="text-indigo-700/70">
            실제 환경에서는 외부 커머스 서버(B2C)에서 결제 완료 시 호출되는
            Webhook API입니다. 원활한 매출(대시보드) 테스트를 위해 임시로 배치한
            버튼입니다.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-4 bg-white p-4 rounded-md border border-indigo-100">
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500">
                1건의 주문 생성 → 현재 상품 재고 1 차감 → 대시보드 출고액 증가
              </p>
            </div>
            <Button
